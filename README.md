...
```bash
# Démarrer un projet à partir d'une idée (lance le PM, boucle interactive)
...
# Configurer un backend par agent
...
- **L'interactivité est préservée** : chaque agent est lancé avec `opencode run --interactive`, les agents peuvent poser des questions via l'outil `question`.
...
└── cli.py           # start, advance, resume, status, list, config, agents
```
